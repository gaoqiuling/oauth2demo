...
public class LoginFilter extends ClientFilter {
	...
	private void getAccessToken(String code, HttpServletRequest request) {
		Result<AccessTokenResponse> result = Oauth2Utils.getAccessToken(getServerUrl(), getAppId(),
				getAppSecret(), code);
		if (!result.isSuccess()) {
	...
	protected boolean refreshToken(String refreshToken, HttpServletRequest request) {
		Result<AccessTokenResponse> result = Oauth2Utils.refreshToken(getServerUrl(), getAppId(), refreshToken);
		if (!result.isSuccess()) {
	...
	private boolean setAccessTokenInSession(AccessTokenResponse rpcAccessToken, HttpServletRequest request) {
